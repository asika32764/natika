...
use Admin\Field\User\UserListField;
use Admin\Field\User\UserModalField;
	...
	public function define(Form $form)
	{
		...
		$form->wrap('basic', null, function(Form $form)
		{
			...
			$form->add('title', new TextField)
				->label(Translator::translate('admin.user.field.title'))
				->required(true);

			// Alias
			$form->add('alias', new TextField)
				->label(Translator::translate('admin.user.field.alias'));

			// Images
			$form->add('images', new TextField)
				->label(Translator::translate('admin.user.field.images'));

			// URL
			$form->add('url', new TextField)
				->label(Translator::translate('admin.user.field.url'))
				->setValidator(new UrlValidator)
				->set('class', 'validate-url');

			// Example: User List
			$form->add('user_list', new UserListField)
				->label('List Example');

			// Example: User Modal
			$form->add('user_modal', new UserModalField)
				->label('Modal Example');
		});
		...
		$form->wrap('text', null, function(Form $form)
		{
			...
			$form->add('introtext', new TextareaField)
				->label(Translator::translate('admin.user.field.introtext'))
				->set('rows', 10);

			// Fulltext
			$form->add('fulltext', new TextareaField)
				->label(Translator::translate('admin.user.field.fulltext'))
				->set('rows', 10);
		});
		...
		$form->wrap('created', null, function(Form $form)
		{
			...
			$form->add('state', new RadioField)
				->label(Translator::translate('admin.user.field.state'))
				->set('class', 'btn-group')
				->set('default', 1)
			...
			$form->add('version', new TextField)
				->label(Translator::translate('admin.user.field.version'));

			// Created
			$form->add('created', new CalendarField)
				->label(Translator::translate('admin.user.field.created'));

			// Modified
			$form->add('modified', new CalendarField)
				->label(Translator::translate('admin.user.field.modified'));

			// Author
			$form->add('created_by', new TextField)
				->label(Translator::translate('admin.user.field.author'));

			// Modified User
			$form->add('modified_by', new TextField)
				->label(Translator::translate('admin.user.field.modifiedby'));
		});
	}
}
